...
function createHarness(options = {}) {
  ...
  const deps = {
    ...
    getLatestSuggestionForRun:
      options.getLatestSuggestionForRun ||
      (async () => options.latestSuggestionForRun || null),
...
describe('processIncomingMessage', () => {
  ...
  it('retries a failed suggestion send without generating again', async () => {
    resetFailedSendRetries();
    const { deps, provider, autoSendCalls, runs } = createHarness({
      autoSendAllowed: true,
      mode: MODES.AUTO,
      latestSuggestionForRun: {
        id: 'sug-failed',
        status: 'failed',
        sentPlatformMessageId: null,
      },
    });
    runs.push({
      id: 'run-old',
    ...
    const result = await processIncomingMessage(inbound, deps);
    assert.equal(result.id, 'run-old');
    assert.equal(provider.calls, 0);
    assert.equal(autoSendCalls.length, 1);
    assert.equal(autoSendCalls[0].suggestionId, 'sug-failed');
    assert.equal(autoSendCalls[0].edited, false);
  });

  it('does not retry send when the suggestion is already SENT', async () => {
    resetFailedSendRetries();
    const { deps, provider, autoSendCalls, runs } = createHarness({
      autoSendAllowed: true,
      mode: MODES.AUTO,
    ...
    await processIncomingMessage(inbound, deps);
    assert.equal(provider.calls, 0);
    assert.equal(autoSendCalls.length, 0);
  });

  it('does not retry send when sentPlatformMessageId is already set', async () => {
    resetFailedSendRetries();
    const { deps, autoSendCalls, runs } = createHarness({
      autoSendAllowed: true,
      mode: MODES.AUTO,
      latestSuggestionForRun: {
        id: 'sug-partial',
...
describe('quality, time, and names in generate context', () => {
  ...
  it('regenerates once on duplicate_outbound and auto-sends a distinct retry', async () => {
    const generateCalls = [];
    const { deps, autoSendCalls } = createHarness({
      autoSendAllowed: true,
      mode: MODES.AUTO_LOW_RISK,
      persistPendingSuggestion: async (row) => ({ id: 'sug-1', ...row }),
      conversation: {
        id: 'conv-1',
        creatorId: 'cr-1',
        platform: 'maloum',
        ...
        revision: 3,
        lastInboundPlatformMessageId: 'in-2',
        lastInboundAt: '2026-09-09T11:49:00.000Z',
        historyBackfilledAt: '2026-09-09T00:00:00.000Z',
      },
      messages: [
        {
        ...
        },
        {
          ...
          senderRole: 'fan',
          text: 'ok and?',
          sentAt: '2026-09-09T11:49:00.000Z',
        },
      ],
    });
    deps.generateReply = async ({ context }) => {
      generateCalls.push(context);
      ...
      return {
        output: {
          schemaVersion: 1,
          reply: 'Was machst du gerade, und warum schreibst du so schnell?',
          replyEnglish: 'What are you doing, and why are you writing so fast?',
          intent: 'question',
          action: 'TEXT_REPLY',
          suggestedRoute: 'AUTO_SEND',
          requiresHumanReview: false,
    ...
    const result = await processIncomingMessage(
      {
        ...
        platform: 'maloum',
        platformChatId: 'chat-1',
        inboundPlatformMessageId: 'in-2',
      },
      deps
    );
    assert.equal(generateCalls.length, 2);
    assert.ok(
      generateCalls[1].constraints.some((rule) =>
        String(rule).includes('HARD: do not repeat the previous outbound')
      )
    );
    assert.equal(result.status, 'succeeded');
    assert.equal(result.route, ROUTES.AUTO_SEND);
    assert.equal((result.output.flags || []).includes('duplicate_outbound'), false);
    assert.equal(autoSendCalls.length, 1);
  });

  it('leaves an identical dedupe retry in review', async () => {
    const generateCalls = [];
    const { deps, autoSendCalls } = createHarness({
      autoSendAllowed: true,
      mode: MODES.AUTO_LOW_RISK,
  ...
  it('gives a follow-up inbound its own run that can auto-send while the first draft goes stale', async () => {
    const conversation = {
      id: 'conv-1',
      creatorId: 'cr-1',
      platform: 'maloum',
    ...
    const { deps, runs, autoSendCalls } = createHarness({
      autoSendAllowed: true,
      mode: MODES.AUTO_LOW_RISK,
      persistPendingSuggestion: async (row) => ({ id: 'sug-1', ...row }),
      conversation,
    });
    deps.loadMessages = async () => {
      const messages = [
        {
          platformMessageId: 'in-1',
          direction: 'inbound',
      ...
      if (conversation.lastInboundPlatformMessageId === 'in-2') {
        messages.push({
          platformMessageId: 'in-2',
          direction: 'inbound',
          senderRole: 'fan',
          text: 'and another thing',
          sentAt: '2026-09-09T12:00:08.000Z',
        });
      }
      return messages;
    };
    ...
    deps.generateReply = async () => {
      if (firstGenerate) {
        firstGenerate = false;
        conversation.lastInboundPlatformMessageId = 'in-2';
    ...
    const second = runs.find((row) => row.inboundPlatformMessageId === 'in-2');
    assert.equal(first.status, 'succeeded');
    assert.equal(first.route, ROUTES.HUMAN_REVIEW);
    assert.ok(second);
    assert.equal(second.status, 'succeeded');
    assert.equal(second.route, ROUTES.AUTO_SEND);
    assert.equal(autoSendCalls.length, 1);
  });
